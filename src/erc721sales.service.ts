...
export class Erc721SalesService extends BaseService {
  ...
  async getTransactionDetails(
    tx: any,
    ignoreENS: boolean = false,
    ignoreContracts: boolean = true,
  ): Promise<any> {
    ...
    while (true) {
      try {
        ...
        if (
          to !== config.nftx_vault_contract_address &&
          code !== '0x' &&
          ignoreContracts
        ) {
          logger.info(
            `contract detected for ${tx.transactionHash} event index ${tx.index}`,
          );
          return;
        }
        ...
        logger.info(
          `handling ${transactionHash} token ${tokenId} log ${tx.index} — ${transactionDate} - from ${tx.blockNumber}`,
        );
        ...
        const initialTo = to;
        to =
          config.ens && !ignoreENS
            ? ensTo
              ? ensTo
              : this.shortenAddress(to)
            : this.shortenAddress(to);
        from =
          isMint && config.includeFreeMint
            ? 'Mint'
            : config.ens
            ? ensFrom
              ? ensFrom
        ...
        if (
          transaction.to === '0x941A6d105802CCCaa06DE58a13a6F49ebDCD481C' &&
          !tweetRequest.alternateValue
        ) {
          // nftx swap of "inner token" that weren't bought in the same transaction ignore this
          logger.info(
            `nftx swap detected without ETH buy, ignoring ${tx.transactionHash} event index ${tx.index}`,
          );
          return;
        }
        return tweetRequest;
        ...
        logger.info(
          `${tokenId} failed to send, retryCount: ${retryCount}`,
          err,
        );
        retryCount++;
        if (retryCount >= 10) {
